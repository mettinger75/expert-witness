...
import { findOrCreateAttorneyContact } from '@/lib/inquiry-helpers'
import { resolveInquiryContactRole } from '@/lib/constants'

// POST: Create an inquiry portal — contact, case, link, invite, and email in one shot
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { firstName, lastName, email, organizationName, invitationMessage, role } = body
    ...
    // 1. Find or create the contact, through the shared helper rather than a
    // second hand-rolled copy of the same lookup. The inline version this
    // replaces matched on `.eq('email', ...)` — case- and whitespace-sensitive,
    // so `Jane@Firm.com` created a second record for someone already on file —
    // and took whichever row the planner returned first, with no ordering.
    let contactId: string
    try {
      contactId = await findOrCreateAttorneyContact(supabase, {
        firstName,
        lastName,
        email,
        organizationName,
    ...
    // 4. Link contact to case in the role the sender chose. This used to be
    // hardcoded to 'retaining_attorney', which mislabelled every invite sent to
    // a referring agency contact, nurse consultant, or paralegal — and the
    // label sticks, because portal onboarding only ever writes opposing counsel
    // and defendants, never the inquirer's own role. The contract and report
    // flows resolve retaining counsel off this exact value, so a wrong one here
    // surfaces later as the wrong name on a retention agreement.
    const { error: linkError } = await supabase
      .from('case_contacts')
      .insert({
        case_id: newCase.id,
        contact_id: contactId,
        role: resolveInquiryContactRole(role),
        is_primary: true,
      })
