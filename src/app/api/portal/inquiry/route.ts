import { NextRequest, NextResponse } from 'next/server'
import { getSupabaseAdmin } from '@/lib/supabase-admin'
import crypto from 'crypto'
import { caseEmailHeaders, caseEmailSubject } from '@/lib/email-threading'
import { sendPortalInviteEmail } from '@/lib/portal-email'
import { checkRateLimit, clientIp } from '@/lib/rate-limit'

// POST: Create an inquiry portal — contact, case, link, invite, and email in one shot
export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const { firstName, lastName, email, organizationName, invitationMessage } = body

    if (!firstName || !lastName || !email) {
      return NextResponse.json(
        { error: 'Missing required fields: firstName, lastName, email' },
        { status: 400 }
      )
    }

    // Public, unauthenticated endpoint — throttle by IP and by submitted email
    // so it can't be used to mass-create inquiry cases or spam inboxes (each
    // request creates a contact + case + invite and sends two emails). Uses the
    // same DB-backed limiter as the other public portal routes.
    const ip = clientIp(request)
    const [ipOk, emailOk] = await Promise.all([
      checkRateLimit(`inquiry:ip:${ip}`, 5, 3600),
      checkRateLimit(`inquiry:email:${String(email).toLowerCase()}`, 3, 3600),
    ])
    if (!ipOk || !emailOk) {
      return NextResponse.json(
        { error: 'Too many requests. Please try again later or contact us directly.' },
        { status: 429 }
      )
    }

    const supabase = getSupabaseAdmin()

    // 1. Check for existing contact with same email to avoid duplicates
    let contactId: string
    const { data: existingContact } = await supabase
      .from('contacts')
      .select('id')
      .eq('email', email)
      .eq('is_active', true)
      .limit(1)
      .single()

    if (existingContact) {
      contactId = existingContact.id
    } else {
      const { data: newContact, error: contactError } = await supabase
        .from('contacts')
        .insert({
          contact_type: 'attorney',
          first_name: firstName,
          last_name: lastName,
          email,
          organization_name: organizationName || null,
        })
        .select('id')
        .single()

      if (contactError || !newContact) {
        console.error('Contact creation error:', contactError)
        return NextResponse.json({ error: 'Failed to create contact' }, { status: 500 })
      }
      contactId = newContact.id
    }

    // 2. Create minimal inquiry case (case_number auto-generated by DB trigger)
    const caseName = `Inquiry — ${firstName} ${lastName}`

    const { data: newCase, error: caseError } = await supabase
      .from('cases')
      .insert({
        case_name: caseName,
        case_type: 'other',
        // `side` is deliberately left NULL. The public inquiry form never asks
        // who the inquiring attorney represents, and defaulting to 'plaintiff'
        // silently mislabels every defense inquiry. `side` feeds conflict
        // checks and report headers, so an unknown side has to stay unknown
        // until counsel supplies it through portal onboarding (which writes it
        // via /api/portal/[token]/case-details). The column is nullable.
        status: 'inquiry',
        priority: 'normal',
        date_of_referral: new Date().toISOString().split('T')[0],
      })
      .select('id, case_number')
      .single()

    if (caseError || !newCase) {
      console.error('Case creation error:', caseError)
      return NextResponse.json({ error: 'Failed to create case' }, { status: 500 })
    }

    const caseNumber = newCase.case_number

    // 3b. Send new-case notification to Dr. Ettinger — THREAD ROOT for the case
    const resendKey = process.env.RESEND_API_KEY
    if (resendKey) {
      const notifyAppUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://markettingermd.com'
      const rootSubject = caseEmailSubject(caseNumber, caseName, true)
      const rootHeaders = caseEmailHeaders(newCase.id, { isRoot: true })
      fetch('https://api.resend.com/emails', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${resendKey}` },
        body: JSON.stringify({
          from: 'Mark Ettinger, M.D. <mark@markettingermd.com>',
          to: 'markettingermd@gmail.com',
          subject: rootSubject,
          headers: rootHeaders,
          html: `
            <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto;">
              <div style="background-color: #0E1F35; color: white; padding: 24px 32px;">
                <h1 style="margin: 0; font-size: 20px; color: #DFC06A;">New Inquiry Case</h1>
              </div>
              <div style="padding: 32px; border: 1px solid #e5e7eb; border-top: none;">
                <p style="color: #374151; font-size: 15px; line-height: 1.6;">
                  A portal invite was sent to <strong>${firstName} ${lastName}</strong>${organizationName ? ` (${organizationName})` : ''} at <strong>${email}</strong>.
                </p>
                <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                  <tr><td style="padding: 8px 0; color: #6b7280; font-size: 14px; width: 120px;">Case:</td><td style="padding: 8px 0; font-size: 14px; font-weight: 600;">${caseName}</td></tr>
                  <tr><td style="padding: 8px 0; color: #6b7280; font-size: 14px;">Number:</td><td style="padding: 8px 0; font-size: 14px;">${caseNumber}</td></tr>
                </table>
                <div style="text-align: center; margin: 28px 0;">
                  <a href="${notifyAppUrl}/cases/${newCase.id}" style="display: inline-block; background-color: #0E1F35; color: white; text-decoration: none; padding: 12px 32px; border-radius: 6px; font-size: 14px; font-weight: 600;">View Case</a>
                </div>
              </div>
            </div>`,
        }),
      }).catch(err => console.error('Inquiry notification email error:', err))
    }

    // 4. Link contact to case
    const { error: linkError } = await supabase
      .from('case_contacts')
      .insert({
        case_id: newCase.id,
        contact_id: contactId,
        role: 'retaining_attorney',
        is_primary: true,
      })

    if (linkError) {
      console.error('Case-contact link error:', linkError)
    }

    // 5. Create portal invite with inquiry onboarding steps
    const token = crypto.randomBytes(32).toString('hex')
    const expiresAt = new Date()
    expiresAt.setDate(expiresAt.getDate() + 90)

    const onboardingSteps = {
      review_fee_schedule: 'pending',
      review_cv: 'locked',
      enter_case_details: 'locked',
      invite_colleague: 'locked',
      schedule_call: 'locked',
      sign_contract: 'not_applicable',
      retainer_payment: 'not_applicable',
      upload_documents: 'locked',
    }

    const { data: invite, error: inviteError } = await supabase
      .from('portal_invites')
      .insert({
        case_id: newCase.id,
        contact_id: contactId,
        token,
        onboarding_mode: true,
        onboarding_steps: onboardingSteps,
        can_view_summary: true,
        can_view_timeline: true,
        can_message: true,
        can_view_reports: false,
        can_edit_reports: false,
        can_upload_documents: true,
        can_view_fee_schedule: true,
        can_view_depositions: false,
        can_view_billing: false,
        can_book_scheduling: true,
        can_sign_contract: false,
        can_invite_contacts: true,
        invitation_message: invitationMessage || null,
        expires_at: expiresAt.toISOString(),
      })
      .select('id')
      .single()

    if (inviteError || !invite) {
      console.error('Portal invite creation error:', inviteError)
      return NextResponse.json({ error: 'Failed to create portal invite' }, { status: 500 })
    }

    // 6. Send the inquiry invite through the shared portal-invite sender.
    // This email used to be built inline here, which is exactly how it drifted
    // out of sync with the shared version — the inline copy still rendered
    // `invitationMessage` as an italic pull-quote attributed back to Mark, and
    // he does not quote himself inside his own email. Keeping one builder keeps
    // the body, greeting, and voice rules in a single place. `subject` and
    // `headers` preserve the case threading the inline send did by hand, so the
    // invite still joins the conversation seeded by the notification above.
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://markettingermd.com'
    const portalUrl = `${appUrl}/portal/${token}`

    const emailResult = await sendPortalInviteEmail({
      recipientEmail: email,
      recipientName: `${firstName} ${lastName}`,
      portalUrl,
      invitationMessage: invitationMessage || undefined,
      isInquiry: true,
      subject: caseEmailSubject(caseNumber, caseName, true),
      headers: caseEmailHeaders(newCase.id, { selfLabel: 'invite' }),
    })

    if (!emailResult.success) {
      // Don't fail the whole request if email fails — the portal URL still works.
      console.error('Inquiry invite email error:', emailResult.error)
    }

    return NextResponse.json({
      caseId: newCase.id,
      contactId,
      portalUrl,
      caseNumber,
      inviteId: invite.id,
    })
  } catch (error) {
    console.error('Inquiry portal creation error:', error)
    return NextResponse.json({ error: 'Failed to create inquiry portal' }, { status: 500 })
  }
}
